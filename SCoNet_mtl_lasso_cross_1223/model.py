...
class MTL(object):
    def __init__(self, config, sess):
        ...
        self.lasso_weight = config['lasso_weight']
    ...
    def build_model_joint_training(self):
        ...
        for h in xrange(1, self.cross_layers+1):
            ...
            sharedHs_list.append(self.shared_Hs[h])  # match the dimensions
        l1_regularizer = tf.contrib.layers.l1_regularizer(scale=self.lasso_weight, scope=None)
        self.loss_lasso = tf.contrib.layers.apply_regularization(l1_regularizer, sharedHs_list)
        self.loss_joint = self.weights_app_news[0] * self.loss_app_joint + self.weights_app_news[1] * self.loss_news_joint
        grads_and_vars = self.opt_joint.compute_gradients(self.loss_joint + self.loss_lasso, params)
        clipped_grads_and_vars = [(tf.clip_by_norm(gv[0], self.max_grad_norm), gv[1]) for gv in grads_and_vars]
    ...
    def train_model(self):
        ...
        if num_batches_app < num_batches_news:  # app smaller than news, and hence single training on news first
            ...
            for _ in batches_single:
                ...
                _, loss_news, self.step = self.sess.run([self.optim_news,
                                                    self.loss_news_only,
                                                    self.global_step],
                                                    feed_dict={
            ...
            for _ in batches_single:
                ...
                _, loss_app, self.step = self.sess.run([self.optim_app,
                                                    self.loss_app_only,
                                                    self.global_step],
                                                    feed_dict={
                                                        k:v for k,v in zip(keys, values)
        ...
        for _ in batches_join:
            ...
            _, loss_joint, loss_lasso, loss_app, loss_news, self.step = self.sess.run([self.optim_joint,
                                                self.loss_joint,
                                                self.loss_lasso,
                                                self.loss_app_joint,
            ...
            cost_joint += np.sum(loss_joint)
            cost_lasso += np.sum(loss_lasso)
            cost_total += np.sum(loss_joint)
            cost_total += np.sum(loss_lasso)
            cost_app += np.sum(loss_app)
            cost_news += np.sum(loss_news)
        if self.show: bar_join.finish()
        return [cost_total/num_batches_max/self.batch_size,
                cost_joint/num_batches_min/self.batch_size,
                cost_lasso/num_batches_min/self.batch_size,
    ...
    def run(self):
        ...
        self.para_str = 'time'+str(int(time.time())) + '_' + 'eu'+str(self.edim_u)+'ev'+str(self.edim_v) \
                        + str(self.layers) + 'lasso'+str(self.lasso_weight) + 'batch'+str(self.batch_size) \
                        + 'wtask'+str(self.weights_app_news) \
        ...
        with open('results_' + self.para_str + '.log', 'w') as ofile:
            ...
            for idx in xrange(self.nepoch):
                ...
                train_loss_total, train_loss_joint, train_loss_lasso, train_loss_app, train_loss_news = self.train_model()
                ...
                print('{:.1f}s. epoch={}, loss_total={:.6f}, loss_joint={:.6f}, loss_lasso={:.6f}, val_l={:.6f}. {:.1f}s'.format(
                        train_time, idx, train_loss_total, train_loss_joint, train_loss_lasso, valid_loss, valid_time))
                ...
                ofile.write('{:.1f}s. epoch={}, loss_total={:.6f}, loss_joint={:.6f}, loss_lasso={:.6f}, val_l={:.6f}. {:.1f}s\n'.format(
                        train_time, idx, train_loss_total, train_loss_joint, train_loss_lasso, valid_loss, valid_time))
